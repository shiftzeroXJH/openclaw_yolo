...
export function CreateExperimentDialog({ onClose, onCreated }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Todo: Handle 'needs_dataset_yaml' if backend sends it. 
      // For now assume READY.
      if (res.experiment_id) {
        onCreated(res.experiment_id)
      }
    } catch (err: any) {
      setError(err?.detail?.error || 'Failed to create experiment')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="card" style={{ width: '500px', maxWidth: '100%' }}>
        ...
        <form className="flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <div className="flex-col gap-2">
            ...
            <input required className="input" value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="例如：初始基准测试" />
          </div>
